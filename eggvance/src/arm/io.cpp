...
void WaitControl::update()
{
    ...
    for (uint x = 0; x < 2; ++x)
    {
        ...
        cycles_word[0x8 + x][0] = kNonSeq[ws0_n] + kWs0Seq[ws0_s] + 1;
        cycles_word[0x8 + x][1] = 2 * kWs0Seq[ws0_s] + 1;
        cycles_word[0xA + x][0] = kNonSeq[ws1_n] + kWs1Seq[ws1_s] + 1;
        cycles_word[0xA + x][1] = 2 * kWs1Seq[ws1_s] + 1;
        cycles_word[0xC + x][0] = kNonSeq[ws2_n] + kWs2Seq[ws2_s] + 1;
        cycles_word[0xC + x][1] = 2 * kWs2Seq[ws2_s] + 1;
